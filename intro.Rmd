...
>1. Some basic R. Not (at all) exaustive. 
    ...
    * Check out this excellent set of [R introduction slides courtesy of UCLA Statistical Consulting Group](http://statistics.ats.ucla.edu/stat/r/seminars/intro.htm)
>2. Some quizzes
...
>* Go to www.r-project.org and download R for your OS
>* Go to www.rstudio.org and download RStudio for your OS
...
>* Roger Koenker: *Buy a case of wine with what you've saved*


# The R Learning Curve
>* is steep.
>* However, support is outstanding.
	* R has a very large and active user community
	* googling your question almost always returns something useful
	* get familiar with [stackoverflow](http://www.stackoverflow.com)
>* If you know other software, switching is easy:
...
* create a 2x2 matrix with normal random numbers z. type ```help(rnorm)``` or ```help(matrix)``` if needed.
...
* install a package simply by typing, for example ```install.packages("ggplot2")```
...
* No! We want to know "Freq" by those categories.
...
```{r}
xtabs(Freq ~ Admit + Dept,data=UCB)	# Freq is explained by Admit and Gender
...
* [click here to download the data from moodle in a zip](https://moodle.ucl.ac.uk/mod/resource/view.php?id=1145211)
* zip contains 4 files, 2 in .csv and 2 in .dta format
...
* google "R data.table"
* you can easily learn this yourself, too little time here
...
* more functionality from other packages:
  * `library(ggplot2)`
  * `library(lattice)`
* one could easily fill 2 hours talking about ggplot alone. Scratching the surface here.
...
```{r qr1,message=FALSE,cache=TRUE}
library(quantreg)
...
* don't copy and paste tables into word (or similar)
