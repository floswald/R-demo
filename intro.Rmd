...
df2$values <- runif(n=nrow(df2)) # but change the entries for "values"
...
```{r demo12}
dat$age.cat <- cut_interval(dat$age,length=5) # add col with age category
dat$ced.cat <- cut_interval(dat$ageced,length=5) # add col with age at educ completion category
str(dat$age.cat) # is a factor?
table(dat$age.cat)
table(dat$ced.cat)
```
...
```{r demo22,cache=TRUE,message=FALSE}
library(reshape)
